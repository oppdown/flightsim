...
test("server-renders the public Flight Lab site", async () => {
  ...
  assert.match(html, /v0\.1\.005/);
  assert.doesNotMatch(html, /src="\/flight-simulator\.html"/);
  assert.doesNotMatch(html, /Every build is tested before it ships\.|View source on GitHub/);
...
test("the public site keeps the simulator asset and source history", async () => {
  ...
  await access(new URL("../public/downloads/flight-lab-pwa-v0.1.005.zip", import.meta.url));
  ...
  assert.match(downloadPage, /flight-lab-pwa-v0\.1\.005\.zip/);
  ...
  assert.match(changelog, /## v0\.1\.005/);
